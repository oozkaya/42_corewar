...
| **`ld`**→→`DIR`⎮`IND`<br />`REG` | Loads a value from an address into a register.<br /><br />*Example: `ld 34,r3` stores `REG_SIZE` octets from 34 bytes after the current address (`PC + (34 % IDX_MOD)`) into the register `r3`.*| 5 | Yes | Yes | 4 |
