...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/04 16:11:40 by oozkaya           #+#    #+#             */
/*   Updated: 2019/02/27 12:45:32 by oozkaya          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_fork(t_env *env, t_proc *proc)
{
	...
	if (proc->working && !proc->charge)
	{
		...
		if (dir & 0x8000)
			dir = dir - 0xFFFF - 1;
		new->pc = (proc->pc + (dir % IDX_MOD)) % MEM_SIZE;
		if (new->pc < 0)
			new->pc += MEM_SIZE;
		printer(env, 0, "P %4d | fork %d (%d)\n", proc->nbr, dir, new->pc);
